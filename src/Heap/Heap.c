    ...
    while(true){
        if (current->length >= size){
            if (current->length > size + sizeof(MemorySegment)){
                MemorySegment* newSegment = (MemorySegment*)((uint32_t)current + sizeof(MemorySegment) + size);//making a new segment
                newSegment->isfree            = true;//giving it values
                newSegment->length          = (((uint32_t)current->length) - sizeof(MemorySegment) +size);
...
void free(void* tofree){
    ...
    if(ams->IsAligned){
    current = (MemorySegment*)(uint32_t)ams->MemorySegmentAddr;//establishing the target but its aligned
    }
    else{
...
void* realloc(void* old,uint32_t size){
    ...
    if(ams->IsAligned){
        oldseg = (MemorySegment*)(uint32_t)ams->MemorySegmentAddr;//establishing the target but its aligned
    }
    else{
    oldseg = ((MemorySegment*)old) - 1;//establishing the target
    }
    //recording the smaller size
    uint32_t smallersize = size;
...
void* aligned_alloc(uint32_t alignment,uint32_t size)
{
    ...
    uint32_t aremainder = alignment % 8;
    ...
    uint32_t address = (uint32_t)mallocval;

    //allign the address
    uint32_t remainder2 = address % alignment;
    address -= remainder2;
